 ...
 * @ht: the hash table for insertion
 * @key: the key
 * @value: the value
 *
...
int hash_table_set(hash_table_t *ht, const char *key, const char *value)
{
	...
	if (ht == NULL || key == NULL || *key == '\0' || value == NULL)
		return (0); /* insertion fail: invalid parameter */

	node = calloc(1, sizeof(hash_node_t));
	if (node == NULL)
		return (0); /* insertion fail: memory allocation failed */
...
/**
 * handle_collision - handles both collision & inserting into the hash table
