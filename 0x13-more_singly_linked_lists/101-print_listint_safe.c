...
/**
 * print_listint_safe - It prints a linked list and checks for a loop
...
size_t print_listint_safe(const listint_t *head)
{
	...
	if (head == NULL)
	{
		return (0); /* If the list is already empty */
	}
	...
	while (head != NULL)
	{
		...
		{
			/*
			 * checks if the current node has been visited already
		...
		/* It keeps track of the nodes visited so far */
		visited_nodes = append_to_list(visited_nodes, node_count, head);

		/* It print current node's information */
		printf("[%p] %d\n", (void *)head, head->n);
...
/**
 * append_to_list - It keeps track of all nodes visited in a linked list
		...
		size_t node_count, const listint_t *new_node)
{
	...
	{
		exit(98); /* For when memory allocation fails */
	}

	/* It append the nodes to the list */
	for (i = 0; i < node_count - 1; i++)
	{
		visited_nodes[i] = node_list[i];
	}
	visited_nodes[i] = new_node; /* It append the new node to the list */

	free(node_list); /* It free the previous memory */
